            ...
            for j in range(len(ScaffoldingMatrix)):
                for k in range(len(ScaffoldingMatrix)):
                    ...
                    if color != 'red' and color != 'white':# and color != 'blue' and color != 'white':
                        scatter1 = ax.scatter(i, j, k, c=color, marker='s', s=60, linewidth=0.5, label=color)  # para mapa continuo sin s y sin linewidth
...
#         plt.draw()
    ...
    def plotEpisodes(self, graph, Lsup, Linf):
        ...
        ax.grid()
        # ax.set_zlabel('Distance Basket - Cylinder')
        plt.title('Representative executions from ' + str(Linf) + ' to ' + str(Lsup), size=18.0)
        for i in range(len(graph)):
            if i <= Lsup and i > Linf:
                # color = ((i-Linf)/(Lsup-Linf), 0.5, 0.5)
                sens1 = []
    ...
    def saveData(self):  # , action, seed):
        f = open('SimulationDataLongExecMultipleCorrBis_TracesVF_OtroReward.pckl', 'wb')
    ...
    def loadData(self):
        f = open('SimulationDataLongExecMultipleCorrBis_TracesVF_VF5.pckl', 'rb')  # SimulationDataMultipleCorrBis
                                                                              # SimulationDataLongExecMultipleCorrBis_TracesVF
                                                                              # SimulationDataLongExecMultipleCorrBis_TracesVF_VF5
