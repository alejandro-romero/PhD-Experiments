...
class Sim(object):
    ...
    def robobo_set_pos(self, (x, y)):
        ...
        # New x, y position
        new_x = x - w / 2 * math.cos(self.robobo.angle * math.pi / 180) + h / 2 * math.sin(
            self.robobo.angle * math.pi / 180)
        new_y = y - w / 2 * math.sin(self.robobo.angle * math.pi / 180) - h / 2 * math.cos(
            self.robobo.angle * math.pi / 180)

        # Check if the new robobo position is inside its movement limits
        if self.check_limits((x, y), 'robobo'):
            self.robobo.xy = (new_x, new_y)
            self.robobo_act.xy = (new_x + w * math.cos(self.robobo_act.angle * math.pi / 180),
                                  new_y + w * math.sin(self.robobo_act.angle * math.pi / 180))
        # self.fig.canvas.draw()
    ...
    def robobo_set_angle(self, angle):
        ...
        self.robobo.angle = angle
        self.robobo_act.angle = angle
        self.robobo_set_pos(centre)
        w = self.robobo.get_width()
        self.robobo_act.xy = (self.robobo.get_x() + w * math.cos(self.robobo_act.angle * math.pi / 180),
                              self.robobo.get_y() + w * math.sin(self.robobo_act.angle * math.pi / 180))
    ...
    #     # New x, y position
    #     new_x = x - w / 2 * math.cos(self.baxter_rarm.angle * math.pi / 180) + h / 2 * math.sin(
    #         self.baxter_rarm.angle * math.pi / 180)
    #     new_y = y - w / 2 * math.sin(self.baxter_rarm.angle * math.pi / 180) - h / 2 * math.cos(
    #         self.baxter_rarm.angle * math.pi / 180)
    #
    ...
    #         self.baxter_rarm_act.xy = (new_x + w * math.cos(self.baxter_rarm_act.angle * math.pi / 180),
    #                                    new_y + w * math.sin(self.baxter_rarm_act.angle * math.pi / 180))
    #     self.fig.canvas.draw()
    #
    ...
    #     self.baxter_rarm.angle = angle
    #     self.baxter_rarm_act.angle = angle
    #     self.baxter_rarm_set_pos(centre)
    #     w = self.baxter_rarm.get_width()
    #     self.baxter_rarm_act.xy = (self.baxter_rarm.get_x() + w * math.cos(self.baxter_rarm_act.angle * math.pi / 180),
    #                                self.baxter_rarm.get_y() + w * math.sin(self.baxter_rarm_act.angle * math.pi / 180))
    ...
    #         self.baxter_larm_act.xy = (new_x + w * math.cos(self.baxter_larm_act.angle * math.pi / 180),
    ...
    def robobo_get_pos(self):
        ...
        h = self.robobo.get_height()
        x, y = self.robobo.xy
        x_c = x + w / 2 * math.cos(self.robobo.angle * math.pi / 180) - h / 2 * math.sin(
            self.robobo.angle * math.pi / 180)
        y_c = y + w / 2 * math.sin(self.robobo.angle * math.pi / 180) + h / 2 * math.cos(
            self.robobo.angle * math.pi / 180)
        return x_c, y_c
    ...
    def robobo_act_get_pos(self):
        ...
        x_c = x + w / 2 * math.cos(self.robobo_act.angle * math.pi / 180) - h / 2 * math.sin(
            self.robobo_act.angle * math.pi / 180)
        y_c = y + w / 2 * math.sin(self.robobo_act.angle * math.pi / 180) + h / 2 * math.cos(
            self.robobo_act.angle * math.pi / 180)
        return x_c, y_c

    def robobo_get_angle(self):
        """Return the angle of the Robobo!"""
        return self.robobo.angle
    #
    ...
    #     x_c = x + w / 2 * math.cos(self.baxter_rarm.angle * math.pi / 180) - h / 2 * math.sin(
    #         self.baxter_rarm.angle * math.pi / 180)
    #     y_c = y + w / 2 * math.sin(self.baxter_rarm.angle * math.pi / 180) + h / 2 * math.cos(
    #         self.baxter_rarm.angle * math.pi / 180)
    #     return x_c, y_c
    #
    ...
    #     x_c = x + w / 2 * math.cos(self.baxter_rarm_act.angle * math.pi / 180) - h / 2 * math.sin(
    #         self.baxter_rarm_act.angle * math.pi / 180)
    #     y_c = y + w / 2 * math.sin(self.baxter_rarm_act.angle * math.pi / 180) + h / 2 * math.cos(
    #         self.baxter_rarm_act.angle * math.pi / 180)
    #     return x_c, y_c
    ...
    #     self.baxter_rarm_set_pos((x + vel * math.cos(self.baxter_rarm.angle * math.pi / 180),
    #                               y + vel * math.sin(self.baxter_rarm.angle * math.pi / 180)))
    #     if self.ball_position == 'baxter_rarm':
    #         self.ball_set_pos(self.baxter_rarm_act_get_pos())
    ...
    def move_robobo(self, vel=20):
        ...
        self.robobo_set_pos((x + vel * math.cos(self.robobo.angle * math.pi / 180),
                             y + vel * math.sin(self.robobo.angle * math.pi / 180)))
        if self.ball_position == 'robobo':
            self.ball_set_pos(self.robobo_act_get_pos())

    def robobo_action(self, relative_angle, vel=20):
        """Move robobo with a specific angle and velocity (default 20)"""
        angle = self.robobo.angle + relative_angle
    ...
    def get_sensorization(self):
        ...
        return dist_rob_ball, dist_ball_box1, dist_ball_box2  # ,reward
    ...
    def world_rules(self):
        ...
        if sens[2] < min_dist_box and self.active_goal == 'goal_box2':# and self.ball_position == 'baxter_larm':  # distance ball-box1
            # if self.ball_position == 'baxter_larm':
            #     self.baxter_larm_set_pos(self.box1_get_pos())
            self.ball_position = 'box2'
            # self.ball_set_pos(self.box1_get_pos())
        elif sens[1] < min_dist_box and self.active_goal == 'goal_box1':
    ...
    def set_active_goal(self, iterations):
        "Establish the active goal at each moment"
        if iterations < 15000:
            self.active_goal = 'goal_box1'
        elif iterations < 30000:
            self.active_goal = 'goal_box2'
        elif iterations < 32500:
            self.active_goal = 'goal_box1'
        else:
            self.active_goal = 'goal_box2'
    ...
    def restart_scenario(self):
        ...
        self.box1_set_pos((np.random.randint(100, 2400), np.random.randint(50, 800)))
        # self.box1_set_pos((np.random.randint(1250, 2400), np.random.randint(50, 800)))
        self.box2_set_pos((np.random.randint(100, 2400), np.random.randint(50, 800)))
