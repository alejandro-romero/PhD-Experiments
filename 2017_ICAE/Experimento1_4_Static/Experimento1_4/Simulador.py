...
class Sim(object):
    ...
    def restart_scenario(self):
        self.baxter_larm_set_pos((np.random.randint(1400, 1600), np.random.randint(200, 450)))
        self.baxter_larm_set_angle(90)

        self.ball_set_pos(self.set_pos_dist(self.baxter_larm_get_pos(), 650))
        self.box1_set_pos(self.set_pos_dist(self.baxter_larm_get_pos(), 800))

        self.robobo_set_pos((np.random.randint(100, 2400), np.random.randint(50, 800)))
        ...
        # self.fig.canvas.draw()
        # print "Distances: ", self.get_distance(self.baxter_larm_get_pos(), self.ball_get_pos()), self.get_distance(self.baxter_larm_get_pos(),self.box1_get_pos())

    def set_pos_dist(self, (x1, y1), dist):
        """Set a new object position at a given distance from another"""

        # x2 = x1#np.random.randint(max(0, y1-30), min(y1+30, 800))
        #
        # y2 = self.solve_quadratic_equation(1, -2*y1, (y1**2+math.pow((x2-x1), 2)-dist**2))

        y2 = np.random.randint(50, 800)
        x2 = self.solve_quadratic_equation(1, -2*x1, (x1**2+math.pow((y2-y1), 2)-dist**2))
